    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: "24"
      - name: Run DevAssure
        uses: your-org/devassure-action@v1
        env:
...
- `setup`: none
- `test`: `path`, `source`, `target`, `commit_id`, `url`, `workers`, `headless` (`headless` defaults to `true` and is always passed)
- `run`: `path`, `filter`, `query`, `tag`, `priority`, `folder`, `url`, `workers`, `headless` (`headless` defaults to `true` and is always passed)
